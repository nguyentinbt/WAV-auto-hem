...
Choose the Gender
    [Arguments]    ${gender}
    Run Keyword If    '${gender}' == 'Male'    Click Element    css:#genterWrapper .custom-control:nth-child(1) label.custom-control-label
    ...    ELSE IF    '${gender}' == 'Female'    Click Element    css:#genterWrapper .custom-control:nth-child(2) label.custom-control-label
    ...    ELSE IF    '${gender}' == 'Other'     Click Element    css:#genterWrapper .custom-control:nth-child(3) label.custom-control-label
    ...    ELSE    Fail    Gender '${gender}' is not valid!
    Set Suite Variable    ${gender_result}    ${gender}
...
Select the Hobbies
    ...
    Run Keyword If    '${hobby}' == 'Sports'    Click Element    css:label[for="hobbies-checkbox-1"]
        ...    ELSE IF    '${hobby}' == 'Reading'    Click Element    css:label[for="hobbies-checkbox-2"]
        ...    ELSE IF    '${hobby}' == 'Music'      Click Element    css:label[for="hobbies-checkbox-3"]
        ...    ELSE    Fail    Hobby '${hobby}' is not supported!
    END
    Set Suite Variable    ${hobbies_result}    ${list_hobbies}
...
Verify that the required fields will be red-highlighted
    ...
    FOR    ${id}    IN    @{fields}
        Wait Until Keyword Succeeds    5s    0.5s    Field Border Should Be Red    ${id}
    END

Field Border Should Be Red
    [Arguments]    ${id}
